/* ============================================================
   D'FRUTA MADRE — UI PRIMITIVES (iconos, logo, helpers)
   Exporta a window
   ============================================================ */
const { useState, useEffect, useRef } = React;

/* ---- Iconos (línea simple, redondeada) ---- */
const I = {
  wa: (p) => (
  ...
  plus: (p) => (
    <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" {...p}>
  ...
  arrow: (p) => (
    <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" {...p}>
  ...
  send: (p) => (
    <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" {...p}>
      <path d="M21 3 9.8 14.2M21 3l-8.5 18.5-2.7-7.3L3 11.5 21 3z"/>
    </svg>
  ),
  cart: (p) => (
    <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...p}>
      <circle cx="9" cy="20" r="1.4"/><circle cx="18" cy="20" r="1.4"/><path d="M2 3h3l2.4 12.4a1.5 1.5 0 0 0 1.5 1.2h8.2a1.5 1.5 0 0 0 1.5-1.2L22 7H6"/>
    </svg>
  ),
  pencilPlus: (p) => (
    <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" {...p}>
...
function Stars({ n = 5 }) {
  return (
    <div className="stars" aria-label={`${n} de 5 estrellas`} style={{ display: 'inline-flex', gap: 2, color: '#F6B73C', fontSize: '1rem' }}>
      {Array.from({ length: 5 }).map((_, i) => (
        <I.star key={i} style={{ opacity: i < n ? 1 : 0.25 }} />
      ))}
    </div>
  );
}
...
function Logo({ size = 1, light = false }) {
  return (
    <span className="logo" style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
      <img src="assets/mascot-fresa.png" alt="" style={{ width: 34 * size, height: 34 * size, objectFit: 'contain' }} />
      <span style={{ lineHeight: 0.86, display: 'inline-block' }}>
        <span style={{
          fontFamily: 'var(--font-display)', fontSize: `${1.18 * size}rem`,
          color: light ? '#fff' : 'var(--coral)', display: 'block', letterSpacing: '.5px',
        }}>D'Fruta</span>
        <span style={{
          fontFamily: 'var(--font-display)', fontSize: `${1.18 * size}rem`,
          color: light ? '#fff' : 'var(--verde-700)', display: 'block', letterSpacing: '.5px',
          marginTop: -2,
        }}>Madre</span>
      </span>
    </span>
...
function useReveal() {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const els = el.matches?.('[data-reveal]') ? [el] : el.querySelectorAll('[data-reveal]');
...
Object.assign(window, { I, Stars, Logo, useReveal });
